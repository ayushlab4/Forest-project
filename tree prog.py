...
class Nature:
    ...
    def create_forest(self, size, y_value):
        ...
        for i in range(iterations):
            ...
            on_dirt_path = (x_value > ((y_value - self.y_intercept) / self.slope) - path_offset and x_value < (
                        (self.y_intercept - y_value) / self.slope) + path_offset)
            if not on_dirt_path:
                x_specific = x_value + random.randint(-50,
                                                      0)  # Add a little randomness so trees aren't too evenly spaced out
                scale = size * random.randint(95, 100) / 100.0  # Add slight, random variances in size
                y_specific = y_value - (size - 1.25) * 20 + random.randint(-5,
                                                                           5)  # Add slight, random offsets in y position for a given row
